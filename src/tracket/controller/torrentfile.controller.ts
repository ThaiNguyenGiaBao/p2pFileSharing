...
class FileController {
  ...
  static async register(
    ...
    try {
      const file = await pool.query("SELECT id FROM file WHERE fname = $1", [
        filename,
      ]);
      ...
      if (file.rows.length) {
        ...
        if (peerFileR.rows.length) {
          res.status(400).json({ message: "Peer already has the file" });
          return;
        }
        ...
        console.log("File registered by " + peerId + "::", file.rows[0]);
        ...
        console.log("File registered by " + peerId + "::", updateFile.rows[0]);
